...
def get_activations(files, model, batch_size=50, dims=2048,
    ...
    if batch_size > len(files):
        ...
        print("length batch size:", batch_size)
...
def get_activations_dataset(cfg, model, batch_size=50, dims=2048,
    ...
    for idx, x in tqdm(enumerate(batches)):
        batch = torch.from_numpy(x).type(torch.FloatTensor)
        if cuda:
            batch = batch.to(device)
        res = model(batch)[0]
...
def get_activations_given_dataset(cfg, dataset, model, batch_size=50, dims=2048,
                                  cuda=False, verbose=False, device=torch.device("cpu"), num_images=50000):
    """Calculates the activations of the pool_3 layer for all images.
    Params:
    -- files       : List of image files paths
...
def get_activations_generate(cfg, model_s, model, batch_size=50, dims=2048,
                             cuda=False, verbose=False, device=torch.device("cpu"), num_images=50000):
    """Calculates the activations of the pool_3 layer for all images.
    Params:
    -- files       : List of image files paths
    ...
    for i in range(0, num_images, batch_size):
        ...
        images = images.data.cpu().numpy() * 0.5 + 0.5
...
def calculate_activation_statistics_given_dataset(cfg, dataset, model, batch_size=50,
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations_given_dataset(cfg, dataset, model, batch_size, dims, cuda, verbose, device, num_images)
    mu = np.mean(act, axis=0)
    sigma = np.cov(act, rowvar=False)
    return mu, sigma


def calculate_activation_statistics_generate(cfg, model_s, model, batch_size=50,
                                             dims=2048, cuda=False, verbose=False, device=torch.device("cpu"),
                                             num_images=50000):
    """Calculation of the statistics used by the FID.
    Params:
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations_generate(cfg, model_s, model, batch_size, dims, cuda, verbose, device, num_images)
    mu = np.mean(act, axis=0)
    sigma = np.cov(act, rowvar=False)
    return mu, sigma
...
def _compute_statistics_of_given_dataset(cfg, dataset, model, batch_size, dims, cuda, device, num_images):
    m, s = calculate_activation_statistics_given_dataset(cfg, dataset, model, batch_size,
                                                         dims, cuda, device=device, num_images=num_images)

    return m, s


def _compute_statistics_of_generate(cfg, model_s, model, batch_size, dims, cuda, device, num_images):
    m, s = calculate_activation_statistics_generate(cfg, model_s, model, batch_size,
                                                    dims, cuda, device=device, num_images=num_images)
...
def calculate_fid_given_dataset(cfg, dataset, model_s, batch_size, cuda, dims, device, num_images):
    """Calculates the FID"""
    ...
    m1, s1 = _compute_statistics_of_given_dataset(cfg, dataset, model, batch_size,
...
def calculate_fid_given_dataset_sanity(cfg, dataset, model_s, batch_size, cuda, dims, device, num_images):
    ...
    path = '/local/mparmis/datasets_intro/data/data_train'
...
if __name__ == '__main__':
    ...
    #gen_img_save_path = str(Path.home()) + '/../../mnt/data/tal/celebhq_256_generated'
    #real_img_save_path = str(Path.home()) + '/../../mnt/data/tal/celebhq_256'
    gen_img_save_path = '/local/mparmis/datasets_intro/data/generated'
    real_img_save_path = '/local/mparmis/datasets_intro/data/data_test'
    paths = [real_img_save_path, gen_img_save_path]
    fid = calculate_fid_given_paths(paths, batch_size=32, cuda=3, dims=2048, device=device)
    print(fid)

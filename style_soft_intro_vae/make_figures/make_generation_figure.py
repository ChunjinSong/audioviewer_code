...
def sample(cfg, logger):
    ...
    print("seed:", seed)
    with torch.no_grad():
        path = '/local/mparmis/AudioViewer/Soft_Intro_VAE/soft-intro-vae-pytorch/style_soft_intro_vae/make_figures/output_scj'
        os.makedirs(path, exist_ok=True)
        os.makedirs(os.path.join(path, cfg.NAME), exist_ok=True)
        draw_uncurated_result_figure(cfg, '/local/mparmis/AudioViewer/Soft_Intro_VAE/soft-intro-vae-pytorch/style_soft_intro_vae/make_figures/output_scj/%s/generations.jpg' % cfg.NAME,
                                     model, cx=0, cy=0, cw=im_size, ch=im_size, rows=6, lods=[0, 0, 0, 1, 1, 2], seed=seed)


if __name__ == "__main__":
    gpu_count = 1
    run(sample, get_cfg_defaults(), description='SoftIntroVAE-generations', default_config='/local/mparmis/AudioViewer/Soft_Intro_VAE/soft-intro-vae-pytorch/style_soft_intro_vae/configs/celeba-hq256.yaml',
